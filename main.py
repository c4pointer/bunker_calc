...
try:
    ...
    conn = sqlite3.connect(
        (file_location_detect+'/Documents/myapp/Bunker_calc1.db'), check_same_thread=False)
except sqlite3.OperationalError as e:
    name_off_app = "BunkerCalc"
    conn = sqlite3.connect(
        (file_location_detect+'/Bunker_calc1.db'), check_same_thread=False)
cur = conn.cursor()
...
class App(Frame):
    ...
    def hfo_tanks(self, tk_list, frame):  # Start to build structure of the app
        ...
        for index, names in enumerate(all_tanks):
            ...
            db_commands_calc.calculation(
                self.table_name_var.get(), self.entryWidgets_entered_suond[names])
            ...
            for items in range(len(all_tanks)):
                ...
                self.labelWidgets_Name[-1].grid(row=self.tank_name_row,
                                                column=self.tank_name_column)

                self.entryWidgets_entered_suond[names].grid(
                    row=self.entry_row, column=1, padx=self.padx_for_lbl)

                self.labelWidgets_volumeResult[names].grid(
                    row=self.entry_row, column=2, padx=self.padx_for_lbl)

                self.entryWidgets_Temp[names].grid(
                    row=self.entry_row, column=3, padx=self.padx_for_lbl)

                self.labelWidgets_DefDens_num[names].grid(
                    row=self.entry_row, column=4, padx=self.padx_for_lbl)

                self.entryWidgets_Dens_num[names].grid(
                    row=self.entry_row, column=5, padx=self.padx_for_lbl)

                self.labelWidgets_ResultMT_num[names].grid(
                    row=self.entry_row, column=6, padx=self.padx_for_lbl)
    ...
    def insert_previous_func(self):
        ...
        for names in prev_measurments.name_of_all_tanks:
            ...
            dict_for_insert_temperature[names].set(
                prev_measurments.temperature_insert)
                ...
                try:
                    if 0 < float(new_density) < 1.0760:
                        ...
                        if self.show_info == True:
                            ...
                            try:
                                db_commands_calc.def_dens_modify(
                                    j, new_density)
                                conn.close()
                            except Exception as e:
                                print(f"Erorr: {e}")
        ...
        try:
            ...
            def calc_diff(frame):
                try:
                    ...
                    if str(frame) == ".!frame6":
                        actual = (
                            self.total_widget_Lbl_dict_num['.!frame6']['text'])
                        by_log = float(
                            self.by_log_Entry_dict['.!frame6'].get())
                        print(actual, "-", by_log)
                        diff = float(actual-by_log)
                        diff = (round(diff, 4))
            ...
            self.total_widget_Lbl_dict[frame].grid(
                row=self.lbl_row+2, column=6, pady=2)

            self.total_widget_Lbl_dict_num[frame] = Label(frame,
                                                          width=10, **self.padding2, borderwidth=2, relief="ridge",
                                                          bg="#c6c5c7"
                                                          )
            self.total_widget_Lbl_dict_num[frame].grid(
                row=self.lbl_row+3, column=6, pady=2)
            ...
            self.by_log_Entry_dict[frame].grid(
                row=self.lbl_row+3, column=5, pady=2)
            ...
            self.difference_Lbl_dict[frame].grid(
                row=self.lbl_row+3, column=7, pady=2)
            ...
            self.difference_Btn_dict[frame].grid(
                row=self.lbl_row+2, column=7, pady=2)

            for i, d in enumerate(self.res):
                data = float(d)
                result.append(data)
            total_quantity = round(sum(result), 3)
            self.total_widget_Lbl_dict_num[frame]['text'] = float(
                total_quantity)
    ...
    def new_tank_interface(self):
        ...
        def addtk():
            ...
            succes = Label(self.frame2, text="Succes added " +
                           str(new_tank_name))
    ...
    def edit_tank_interface(self):
        ...
        self.add_type = Button(self.frame3,
                               ...
                               command=lambda: db_commands_calc.type_select_tank(
                                   self.table_name, self.set_type_var.get()),
                               bg="red")
    ...
    def btn_add_sound_val(self):
        ...
        def add_value():
            ...
            try:
                ...
                self.show_error = showerror("Error with input data",
                                            message="In function(btn_add_sound_val) " + str(
                                                e)
                                            )
        ...
        self.addsound = Entry(self.frame3, width=12,
                              textvariable=self.addsound_var)
    ...
    def license_show(self):
        ...
        if self.iterator_for_license == 0:
            ...
            self.close_license = Button(
                self.frame6, command=close, text="Close")
